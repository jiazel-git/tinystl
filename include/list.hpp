...
struct list_node
{
    ...
    explicit list_node( const T& value, node_raw_ptr prev, node_ptr&& next ) : _data( value ), _prev( prev ), _next( std::move( next ) ) {}
    explicit list_node( T&& value, node_raw_ptr prev, node_ptr&& next ) : _data( std::move( value ) ), _prev( prev ), _next( std::move( next ) ) {}
    explicit list_node( const T& value ) : _data( value ), _prev( nullptr ), _next( nullptr ) {}
...
public:
    ...
    const_reference front() const {
        return const_cast< const_reference >( _head->_next->_data );
    }
    reference back() {
        return _tail->_prev->_data;
    }
    const_reference back() const {
        return const_cast< const_reference >( _tail->_prev->_data );
    }
    iterator begin() {
        return get_raw( _head->_next );
    }
    ...
    iterator emplace( const_iterator posion, Args&&... args ) {
        ...
        auto new_node          = std::make_unique< list_node< T > >( value_type( std::forward< Args >( args )... ), prev, std::move( cur ) );
    ...
    void splice( const_iterator posion, list& other ) {
        splice( posion, std::move( other ) );
    }
    void splice( const_iterator posion, list&& other ) {
        splice( posion, std::move( other ), other.cbegin(), other.cend() );
    }
    void splice( const_iterator position, list& other, const_iterator iter ) {
        splice( position, std::move( other ), iter );
    }
    void splice( const_iterator position, list&& other, const_iterator iter ) {
        // todo
    }
...
private:
    static node_raw_ptr get_raw( const node_ptr& ptr ) {
        return ptr.get();
    }
    void init() {
